...
const MapaEscritorios: React.FC<MapaProps> = ({
  ...
  const nonInteractiveStates = ["pr", "mg"];

  return (
    <div className="map-container">
      <svg
        className="map-svg"
        viewBox="105 200 300 300"
      ...
      >
        {states.map((state) => (
          <g
            key={state.id}
            className={`${
              !nonInteractiveStates.includes(state.id) ? "interactive" : ""
            } ${activeState === state.id ? "active" : ""}`}
            onClick={() =>
              !nonInteractiveStates.includes(state.id) &&
              onStateClick(state.id as StateAbbr)
            }
          >
            <path
              d={state.path}
