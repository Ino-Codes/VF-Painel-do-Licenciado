  ...
  return (
    <div className="p-2">
      ...
      <div className="content-area document-center">
        ...
        <div className="tabs">
            <button className={`tab-item ${!selectedCategory ? 'active' : ''}`} onClick={() => setSelectedCategory(null)}>
                Todas
            </button>
          {categories.map((cat) => (
            <button key={cat} className={`tab-item ${selectedCategory === cat ? 'active' : ''}`} onClick={() => setSelectedCategory(cat)}>
              {cat}
            </button>
          ))}
        </div>
            ...
            <div key={faq.id} className="faq-item">
              ...
              {activeAccordion === faq.id && (
                <div className="faq-answer">
                  <p>{faq.answer}</p>
                  {faq.document_url && (
                    <a href={faq.document_url} target="_blank" rel="noopener noreferrer" className="faq-document-link">
                      Baixar Documento: {faq.document_originalname}
                    </a>
                  )}
                  {user.role === 'admin' && (
                     <div className="faq-actions">
                        <button className="list-button delete" onClick={() => handleDeleteClick(faq.id)}>Excluir</button>
                    </div>
                  )}
                </div>
              )}
      ...
      {isModalOpen && <FaqModal onClose={() => setIsModalOpen(false)} onSuccess={() => { setIsModalOpen(false); fetchFaqs(); fetchCategories(); }} />}
      <ConfirmationModal isOpen={isConfirmOpen} onClose={() => setIsConfirmOpen(false)} onConfirm={handleConfirmDelete} title="Confirmar Exclusão" message="Tem certeza que deseja excluir esta pergunta?" />
      <Footer />
    </div>
  );
};
