...
module.exports = function (pool, createNotification) {
  ...
  router.get(
    ...
    checkRole(["admin", "rh"]),
    async (req, res) => {
      // Por padrão, busca o pipeline de Recrutamento
      const { pipeline_type = "Recrutamento" } = req.query;
      try {
        // A query agora precisa encontrar os candidatos que pertencem às etapas do pipeline selecionado
        const result = await pool.query(
          `SELECT c.* FROM candidates c
           JOIN recruitment_stages s ON c.stage_id = s.id
           WHERE s.pipeline_type = $1 AND c.status = 'Ativo'
  ...
  // Converter um candidato em colaborador (mudar pipeline_type para 'Onboarding' e associar a um usuário)
  router.put(
    ...
    async (req, res) => {
      ...
      try {
        ...
        // 1. Encontrar a primeira etapa do pipeline de "Onboarding"
        const onboardingStageResult = await client.query(
          "SELECT id FROM recruitment_stages WHERE pipeline_type = 'Onboarding' ORDER BY stage_order ASC LIMIT 1"
        );

        if (onboardingStageResult.rowCount === 0) {
          await client.query("ROLLBACK");
          return res
            .status(400)
            .json({ error: "Nenhuma etapa de Onboarding foi configurada." });
        }
        const firstOnboardingStageId = onboardingStageResult.rows[0].id;

        // 2. Atualizar o candidato para a nova etapa e associar o user_id
        const updatedCandidate = await client.query(
          "UPDATE candidates SET stage_id = $1, user_id = $2 WHERE id = $3 RETURNING *",
          [firstOnboardingStageId, user_id || null, id]
        );
        ...
        res.status(500).json({ error: "Erro ao converter candidato." });
      } finally {
        client.release();
      }
    }
