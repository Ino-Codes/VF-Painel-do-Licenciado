...
module.exports = function (pool, sgMail, logActivity) {
  /**
   * Rota de Login
   * Autentica um utilizador com base em e-mail e senha.
   */
   ...
   * Gera um token seguro e envia um e-mail com o link de redefinição.
   */
  router.post("/solicitar-redefinicao", async (req, res) => {
    ...
    try {
      ...
      // Se o utilizador existir, procede com o envio do e-mail.
      // Se não, a função termina aqui, mas a mesma mensagem é enviada abaixo.
      if (user) {
        ...
        const hashedToken = crypto
          ...
          .update(resetToken)
          .digest("hex");
        const tokenExpiry = new Date(Date.now() + 900000); // Token válido por 15 minutos
        ...
        // O URL enviado no e-mail contém o token NÃO encriptado
      ...
      // Resposta genérica para evitar que atacantes descubram quais e-mails estão registados
      res.json({
        message:
  ...
  router.post("/redefinir-senha", async (req, res) => {
    ...
    try {
      // Encripta o token recebido para comparar com o que está na base de dados
      const hashedToken = crypto
        .createHash("sha256")
        .update(token)
        .digest("hex");
